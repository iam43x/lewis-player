...
export default function Home() {
  ...
  const ThemeToggle = () => (
    ...
    >
      {c.isDark ? <Sun className="w-4 h-4" style={{ color: c.text }} /> : <Moon className="w-4 h-4" style={{ color: c.text }} />}
    </button>
  );
        ...
        <main className="max-w-6xl mx-auto px-3 sm:px-6 py-4">
          {loading ? (
            <div className="flex items-center justify-center py-20">
              <div className="w-6 h-6 border-2 rounded-full animate-spin" style={{ borderColor: c.chrome, borderTopColor: c.accent }} />
            </div>
          ) : (
